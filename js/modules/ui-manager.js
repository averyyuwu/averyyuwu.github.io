class UIManager {
  constructor(todoManager, todoItemFormatter) {
    ...
    // Sort modal elements
    this.sortModal = document.querySelector(".sort-modal");
    this.sortTasksBtn = document.querySelector(".sort-tasks-btn");
    this.sortBySelect = document.querySelector(".sort-by-select");
    this.sortOrderSelect = document.querySelector(".sort-order-select");
    this.applySortBtn = document.querySelector(".apply-sort-btn");
    this.cancelSortBtn = document.querySelector(".cancel-sort-btn");

    // Add sort state
    this.sortBy = "name";
    this.sortOrder = "asc";

    // Add import confirm modal elements
    this.importConfirmModal = document.querySelector(".import-confirm-modal");
    this.cancelImportBtn = document.querySelector(".cancel-import-btn");
    this.confirmImportBtn = document.querySelector(".confirm-import-btn");

    // Add reminder tab elements
    this.editReminderDate = document.querySelector(".edit-reminder-date");
    this.editReminderTime = document.querySelector(".edit-reminder-time");
    this.editReminderNote = document.querySelector(".edit-reminder-note");
    ...
    // Initialize pending import data
  ...
  addEventListeners() {
    ...
    // Sort modal open/close logic
    this.sortTasksBtn.addEventListener("click", () => this.showSortModal());
    this.cancelSortBtn.addEventListener("click", () => this.hideSortModal());
    this.applySortBtn.addEventListener("click", () => this.handleApplySort());
  ...
  handleEditTodo(id) {
    ...
    this.populateReminderFields(todo);
  ...
  // Add new method to populate reminder fields
  populateReminderFields(todo) {
    if (todo.reminder) {
      this.editReminderDate.value = todo.reminder.date || "";
      this.editReminderTime.value = todo.reminder.time || "";
      this.editReminderNote.value = todo.reminder.note || "";
    } else {
      this.editReminderDate.value = "";
      this.editReminderTime.value = "";
      this.editReminderNote.value = "";
    }

    // Add clear reminder button functionality
    const reminderTab = document.getElementById("reminders-tab");

    // Remove old clear button if it exists
    const oldClearBtn = reminderTab.querySelector(".clear-reminder-btn");
    if (oldClearBtn) {
      oldClearBtn.remove();
    }

    // Add clear button if reminder exists
    if (todo.reminder && (todo.reminder.date || todo.reminder.time)) {
      const clearBtn = document.createElement("button");
      clearBtn.className = "btn btn-error clear-reminder-btn mt-4";
      clearBtn.textContent = "Clear Reminder";
      clearBtn.addEventListener("click", () => {
        this.todoManager.clearReminder(todo.id);
        this.editReminderDate.value = "";
        this.editReminderTime.value = "";
        this.editReminderNote.value = "";
        clearBtn.remove();
        this.showAlertMessage("Reminder cleared", "success");
      });
      reminderTab.appendChild(clearBtn);
    }
  }

  // Add new method to save reminder data
  saveReminderData(taskId) {
    const date = this.editReminderDate.value;
    const time = this.editReminderTime.value;
    const note = this.editReminderNote.value;

    if (date && time) {
      this.todoManager.setReminder(taskId, date, time, note);
      ...
      this.showAlertMessage(
        "Both date and time are required for reminders",
        "error"
      );
    }
  }
  ...
  populateSubTasks(subTasks = []) {
    ...
    subTasks.forEach((subTask) => {
      ...
      subTaskElement.innerHTML = `
        <div class="sub-task-content">
          <div class="todo-item-content">
            ...
            <!-- Progress bar removed from edit modal -->
          </div>
        </div>
        <div class="sub-task-actions">
          <input
            type="number"
            class="input input-bordered input-sm subtask-progress-edit"
            data-parent-id="${subTask.parentId}"
      ...
      // Add event listener for the progress input
      const progressInput = subTaskElement.querySelector(
        ".subtask-progress-edit"
      );
      progressInput.addEventListener("change", (e) => {
        const parentId = e.target.dataset.parentId;
        const subTaskId = e.target.dataset.subtaskId;
        const progress = parseInt(e.target.value, 10);
        this.updateSubTaskProgressInModal(parentId, subTaskId, progress);
      });
    });
  }

  // Add a new method to update subtask progress from the modal
  updateSubTaskProgressInModal(parentId, subTaskId, progress) {
    const parent = this.todoManager.todos.find((t) => t.id === parentId);
    if (!parent || !parent.subTasks) return;

    const subTask = parent.subTasks.find((st) => st.id === subTaskId);
    if (subTask) {
      subTask.progress = Math.max(0, Math.min(100, progress)); // Ensure value is between 0 and 100
      this.todoManager.saveToLocalStorage();
      this.showAlertMessage(
        `Subtask progress updated to ${progress}%`,
        "success"
      );
    }
  }
  ...
  async handleImportData(event) {
    ...
    reader.onload = async (e) => {
      ...
      this.pendingImportData = e.target.result;
      this.showImportConfirmModal();
      // Reset file input
      event.target.value = "";
    };
  ...
  showImportConfirmModal() {
    this.importConfirmModal.classList.add("show");
    document.querySelector(".dim-overlay").classList.add("active");
  }

  hideImportConfirmModal() {
    this.importConfirmModal.classList.remove("show");
    document.querySelector(".dim-overlay").classList.remove("active");
    this.pendingImportData = null;
  }
  ...
  async confirmImportData() {
    ...
    try {
      await appDataManager.importData(this.pendingImportData);
  ...
  handleAddSubTask() {
    ...
    const handleCreate = () => {
      ...
      const progress =
        document.querySelector(".new-subtask-progress").value || 0;
      ...
      const subTask = this.todoManager.addSubTask(
        ...
        this.selectedCategories.join(", ") || "No category",
        displayBar,
        parseInt(progress, 10) // Convert string to integer
      );
  ...
  toggleSubTaskStatus(parentId, subTaskId) {
    const settingsContainer = document.querySelector(".settings-container");
    const isInModal = settingsContainer.classList.contains("active");

    this.todoManager.toggleSubTaskStatus(parentId, subTaskId);

    if (isInModal) {
      // If we're in the edit modal, update the subtasks display directly
      const todo = this.todoManager.todos.find((t) => t.id === parentId);
      if (todo && todo.subTasks) {
        this.populateSubTasks(todo.subTasks);

        // Make sure the dim overlay stays active
        document.querySelector(".dim-overlay").classList.add("active");
      }
    } else {
      // If we're in the main view, apply filters as usual
      this.applyFilters();
    }
  }
  ...
  deleteSubTask(parentId, subTaskId) {
    ...
    // Check if we're in the edit modal
    const settingsContainer = document.querySelector(".settings-container");
    const isInModal = settingsContainer.classList.contains("active");
  ...
  showSortModal() {
    this.sortModal.classList.add("show");
    document.querySelector(".dim-overlay").classList.add("active");
  }

  hideSortModal() {
    this.sortModal.classList.remove("show");
    document.querySelector(".dim-overlay").classList.remove("active");
  }
  ...
  handleApplySort() {
    ...
    this.hideSortModal();
    this.applyFilters();
    this.showAlertMessage("Tasks sorted", "success");
  }

  // Helper to get difficulty value for sorting
  getDifficultyValue(todo) {
    const tags = todo.tag.split(", ");
    if (tags.includes("Easy")) return 1;
    if (tags.includes("Medium")) return 2;
    if (tags.includes("Hard")) return 3;
    return 0;
  }

  // Returns a sorted copy of todos based on current sort state
  getSortedTodos(todos) {
    if (!this.sortBy) return todos;
    const sorted = [...todos];
      ...
      switch (this.sortBy) {
        case "dueDate":
          ...
          if (a.dueDate === "No due date" && b.dueDate === "No due date")
            return 0;
          if (a.dueDate === "No due date") return 1 * order;
          if (b.dueDate === "No due date") return -1 * order;
          return (
            (a.dueDate > b.dueDate ? 1 : a.dueDate < b.dueDate ? -1 : 0) * order
          );
        case "difficulty":
          return (
            (this.getDifficultyValue(a) - this.getDifficultyValue(b)) * order
          );
        case "progress":
          return ((a.progress || 0) - (b.progress || 0)) * order;
        case "name":
          return a.task.localeCompare(b.task) * order;
        default:
          return 0;
      }
    });
